...
export default function load(dir: string): Array<ControllerExtra.ControllerClassType> {
    const files = fs.readdirSync(dir);
    const Controllers: ControllerExtra.ControllerClassType[] = [];
    files.forEach(file => {
        ...
        if (stat.isFile) {
            ...
            if (Class === Controller || proto === Controller || proto instanceof Controller) {
